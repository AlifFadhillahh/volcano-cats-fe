    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-obsidian-3 border border-lava/40 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl animate-bounce-in animate-lava-pulse">
        <div className="text-center mb-4">
          ...
          <p className="text-ash text-sm mt-1">Kamu selamat dari Lava Cat. Taruh balik Lava Cat di posisi mana?</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-center text-xs text-ash mt-1">Deck saat ini: {deckCount} kartu</p>
        </div>

        <button
          onClick={() => onConfirm(position)}
          className="w-full py-3 rounded-xl font-display text-white bg-lava-gradient hover:shadow-lava-glow transition-all active:scale-95"
        >
          🌋 Taruh Lava Cat di Sini
        </button>
      </div>
    </div>
  );
